...
public class GifDecoder {
	...
	// add by lijinglin to record the transparency color
	protected Color lastTransparencyColor = new Color(0, 0, 0, 0);
		...
		if (lastDispose > 0) {
			...
			if (lastImage != null) {
				...
				if (lastDispose == 2) {
					...
					Color c = null;
					if (transparency) {
						c = lastTransparencyColor;// //new Color(0, 0, 0, 0); // assume background is transparent
					} else {
	...
	protected void readImage() {
		...
		if (transparency) {
			...
			// record the transparency color, save is 0xFFrrggbb
			lastTransparencyColor = new Color(save & 0xF00 >> 8, save
					& 0x0F0 >> 4, save & 0x00F);
		}
